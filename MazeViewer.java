   ...
   private static MazeFrame readMazeFile(String fileName) throws IOException {
	   ...
	   for(int i = 0; i < rowNum; i++){
		   ...
		   for(int j = 0; j < colNum; j++){
			   ...
			   }else if(line.charAt(j) == FREE_CHAR){
				   mazeData[i][j] = Maze.FREE;
			   }
		   }
	   }

	   MazeCoord entry = new MazeCoord (in.nextInt(), in.nextInt());
	   MazeCoord exit = new MazeCoord (in.nextInt(), in.nextInt());
	   return new MazeFrame(mazeData, entry, exit);
